 ...
 * @author nguyen
 */
@WebServlet(urlPatterns = {"/admin-save-cart"})
public class SaveCartController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (cart != null) {      
            ...
            Long resultHistory = 0L;
                        Timestamp date = new Timestamp(System.currentTimeMillis());


            for (Map.Entry<Long, String> entry : cart.getCart().entrySet()) {
            }
            if (resultUpdate || (resultHistory != 0)) {
                request.setAttribute("TYPE", "success");
                    request.setAttribute("MESSAGE", "Assign promotion success!");
            }else {
                request.setAttribute("TYPE", "danger");
                    request.setAttribute("MESSAGE", "Some user assign fail!");
            }
        }
